...
class SearchFormView(FormView):
	...
	def get_context_data(self, **kwargs):
		...
		if( query != '' ):
			...
			self.categories = models.Category.objects.filter(Q(title__icontains=query)|Q(description__icontains=query))
			self.tags  		= models.Tag.objects.filter(title__icontains=query)
			# Only retrieve published, non-deleted posts
			self.posts		= models.Post.objects.filter(published__exact=True,deleted__exact=False).filter(Q(title__icontains=query)|Q(body__icontains=query)|Q(pub_date__icontains=query))
...
class PostDetailView(DetailView):
	...
	def get_object(self):
		...
		self.queryset = models.Post.objects.filter(slug__exact=self.kwargs['slug'],published__exact=True,deleted__exact=False)
		# Call the superclass
...
class TagDetailView(DetailView):
	...
	def get_object(self):
		self.queryset = models.Tag.objects.filter(slug__exact=self.kwargs['slug'])
		# Call the superclass
...
class CategoryDetailView(DetailView):
	...
	def get_object(self):
		self.queryset = models.Category.objects.filter(slug__exact=self.kwargs['slug'])
		# Call the superclass
...
class PostListView(ListView):
	...
	queryset = models.Post.objects.filter(published__exact=True,deleted__exact=False)
